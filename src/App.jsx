                     ...
                     <div className="space-y-4 flex-1">
                        {sgTasks.map(task => (
                           <div key={task.id} className="p-5 bg-zinc-50/50 rounded-3xl border border-zinc-200/50 hover:bg-white hover:border-violet-300 hover:shadow-[0_8px_30px_rgb(0,0,0,0.05)] transition-all duration-300 group relative">
                              ...
                              <div className="flex items-center gap-4 bg-white p-3.5 rounded-2xl border border-zinc-200/60 shadow-sm">
                                 <input 
        ...
        <div className="fixed inset-0 bg-zinc-900/40 backdrop-blur-md z-[100] flex items-center justify-center p-4 animate-in fade-in duration-200">
          <div className="bg-white rounded-[2rem] shadow-2xl border border-zinc-100 w-full max-w-sm p-8 animate-in zoom-in-95 duration-200">
            ...
            <div className="flex justify-end gap-3">
              {alertConfig.onConfirm && <button onClick={() => setAlertConfig(null)} className="px-5 py-2.5 text-zinc-600 font-bold hover:bg-zinc-100 rounded-2xl transition-colors text-sm">Cancel</button>}
              <button 
            ...
            <form onSubmit={handleChangePassword} className="space-y-5">
              <div className="relative">
                <input 
                  ...
                  className="w-full p-4 border border-zinc-200 rounded-2xl focus:ring-2 focus:ring-violet-500/20 focus:border-violet-500 outline-none pr-12 transition-all font-semibold text-sm" 
              ...
              <div className="flex justify-end gap-3 mt-8 pt-2">
                <button type="button" onClick={() => setShowPasswordModal(false)} className="px-5 py-2.5 text-zinc-600 font-bold hover:bg-zinc-100 rounded-2xl transition-colors text-sm">Cancel</button>
          ...
          <div className="bg-white rounded-[2rem] shadow-2xl border border-zinc-100 w-full max-w-sm p-8 animate-in zoom-in-95 duration-200">
            ...
            <form onSubmit={handleLogin} className="space-y-4">
              ...
              <div className="flex justify-end gap-3 mt-8 pt-4 border-t border-zinc-100">
                <button type="button" onClick={() => setShowLogin(false)} className="px-5 py-2.5 text-zinc-600 font-bold hover:bg-zinc-100 rounded-2xl transition-colors text-sm">Cancel</button>
          ...
          <div className="bg-white rounded-[2rem] shadow-2xl border border-zinc-100 w-full max-w-md p-8 animate-in zoom-in-95 duration-200">
            ...
            <div className="pt-6 border-t border-zinc-100">
               ...
               <form onSubmit={e => { e.preventDefault(); const t = e.target; addTeamMember(t.name.value, t.role.value); t.reset(); }} className="flex gap-2.5">
                 <input name="name" type="text" placeholder="Name..." className="flex-1 p-3.5 text-sm font-semibold bg-zinc-50 border border-zinc-200 rounded-2xl outline-none focus:bg-white focus:ring-2 focus:ring-violet-500/20 focus:border-violet-500 transition-all" required />
                 <select name="role" className="p-3.5 text-sm font-semibold bg-zinc-50 border border-zinc-200 rounded-2xl outline-none focus:bg-white focus:border-violet-500 transition-all cursor-pointer">
            ...
            <div className="mt-8 flex justify-end">
              <button onClick={() => setShowTeamModal(false)} className="px-6 py-2.5 text-zinc-700 font-bold bg-zinc-100 rounded-2xl hover:bg-zinc-200 transition-colors text-sm">Done</button>
            </div>
          </div>
        </div>
      )}
      ...
      <div className="bg-white rounded-[2rem] shadow-2xl border border-zinc-100 w-full max-w-md p-8 max-h-[90vh] overflow-y-auto animate-in zoom-in-95 duration-200 custom-scrollbar">
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          <div>
            ...
            <input type="text" required value={name} onChange={e => setName(e.target.value)} className="w-full p-4 bg-zinc-50 border border-zinc-200 rounded-2xl focus:bg-white focus:ring-2 focus:ring-violet-500/20 focus:border-violet-500 outline-none transition-all font-semibold text-sm" />
          </div>
          <div>
          ...
          <div>
            ...
            <select value={assignee} onChange={e => setAssignee(e.target.value)} className="w-full p-4 bg-zinc-50 border border-zinc-200 rounded-2xl outline-none focus:bg-white transition-all font-semibold cursor-pointer text-sm">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input type="date" required value={startDate} onChange={e => { setStartDate(e.target.value); if (endDate && e.target.value > endDate) setEndDate(e.target.value); }} className="w-full p-4 bg-zinc-50 border border-zinc-200 rounded-2xl outline-none focus:bg-white transition-all font-semibold text-sm" />
            </div>
            <div>
              <label className="block text-xs font-bold text-zinc-700 mb-1.5 ml-1 uppercase tracking-wider">End Date</label>
              <input type="date" required min={startDate} value={endDate} onChange={e => setEndDate(e.target.value)} className="w-full p-4 bg-zinc-50 border border-zinc-200 rounded-2xl outline-none focus:bg-white transition-all font-semibold text-sm" />
            </div>
          </div>
          ...
          <div className="pt-4 border-t border-zinc-100">
             ...
             <div className="flex items-center gap-3 bg-zinc-50 p-3 rounded-2xl border border-zinc-200 w-max">
          ...
          <div className="flex justify-end gap-3 mt-8 pt-4 border-t border-zinc-100">
            <button type="button" onClick={onClose} className="px-5 py-2.5 text-zinc-600 font-bold hover:bg-zinc-100 rounded-2xl transition-colors text-sm">Cancel</button>
          ...
          <div className="bg-white/95 backdrop-blur-2xl border border-zinc-200/80 rounded-[2rem] shadow-[0_20px_60px_rgba(0,0,0,0.15)] flex flex-col overflow-hidden animate-in slide-in-from-bottom-6 fixed inset-0 sm:relative sm:inset-auto w-full h-full sm:w-[380px] sm:h-[600px] sm:mb-4 z-[70]">
            {!activeChannel ? (
               <div className="flex flex-col h-full">
                 ...
                 <div className="flex-1 overflow-y-auto bg-[#FAFAFA] p-3.5 space-y-2.5 custom-scrollbar">
                    {showCreate && (
...
function Calendar({ tasks, onTaskClick }) {
  ...
  return (
    <div className="flex flex-col min-h-full pb-8">
      <div className="flex justify-between items-center mb-6 flex-shrink-0 sticky top-0 bg-[#FAFAFA]/95 backdrop-blur-md z-20 py-2">
        <button onClick={prevMonth} className="p-2.5 hover:bg-white rounded-2xl shadow-sm border border-zinc-200/80 transition-all text-zinc-700 font-bold">&lt;</button>
        ...
        {weeks.map((week, wIdx) => {
           ...
           return (
              <div key={wIdx} className="grid grid-cols-7 gap-px bg-zinc-200/60 flex-1 min-h-[120px] relative">
                 {week.map((day, dIdx) => (
                    <div key={dIdx} className={`p-2 ${day.isCurrentMonth ? 'bg-white' : 'bg-[#FAFAFA]'}`}>
