...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-zinc-50/50 text-zinc-900 font-sans p-4 md:p-8 selection:bg-violet-200 selection:text-violet-900 pb-28">
      <header className="max-w-7xl mx-auto bg-white/70 backdrop-blur-xl rounded-[2rem] shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-zinc-150/50 p-5 mb-8 flex flex-col md:flex-row justify-between items-center gap-5 sticky top-4 z-40 transition-all">
        <div>
        ...
        <div className="flex flex-wrap gap-2.5 items-center">
          {userRole.role === 'guest' ? (
            ...
            <>
              <div className="flex items-center gap-3 bg-zinc-50 px-4 py-2 rounded-2xl border border-zinc-150/50 hover:bg-zinc-100 transition-colors">
              ...
              {userRole.role === 'manager' && (
                <>
                  <button onClick={() => setShowTeamModal(true)} className="flex items-center gap-2 px-4 py-2.5 text-zinc-700 bg-white border border-zinc-150/80 rounded-2xl hover:bg-zinc-50 hover:border-zinc-300 transition-all shadow-sm font-bold text-sm">
                    <Users size={16} className="text-zinc-500" /> Users
                  </button>
                  <div className="flex bg-white border border-zinc-150/80 rounded-2xl shadow-sm p-1 gap-1">
                    ...
                    <div className="w-px bg-zinc-150 my-1"></div>
              ...
              <button onClick={generatePDF} className="flex items-center gap-2 px-4 py-2.5 text-zinc-700 bg-white border border-zinc-150/80 rounded-2xl hover:bg-zinc-50 hover:border-zinc-300 transition-all shadow-sm font-bold text-sm">
      ...
      <main id="report-content" className="max-w-7xl mx-auto space-y-8">
        <section className="bg-white rounded-[2rem] shadow-[0_10px_40px_rgb(0,0,0,0.03)] border border-zinc-150/50 p-6 lg:p-8">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
            ...
            <div className="flex flex-wrap items-center gap-4">
              {view === 'timeline' && (
                <div className="flex items-center gap-3 bg-zinc-50 px-4 py-2 rounded-2xl border border-zinc-150/60 shadow-sm animate-in fade-in zoom-in duration-200">
                  <span className="text-[10px] font-black text-zinc-400 uppercase tracking-widest">Zoom</span>
                  <input 
                    type="range" min="15" max="100" 
                    value={timelineZoom} 
                    onChange={e => setTimelineZoom(Number(e.target.value))} 
                    className="w-24 h-1.5 bg-zinc-150 rounded-lg appearance-none cursor-pointer accent-violet-600"
                    title="Adjust timeline scale"
                  />
                </div>
              )}
              <div className="flex bg-zinc-50 rounded-2xl p-1.5 border border-zinc-150/60">
                <button onClick={() => setView('timeline')} className={`px-5 py-2 text-xs font-bold rounded-xl transition-all duration-300 ${view === 'timeline' ? 'bg-white shadow-sm text-zinc-900 border border-zinc-150/50' : 'text-zinc-500 hover:text-zinc-700'}`}>Timeline</button>
                <button onClick={() => setView('calendar')} className={`px-5 py-2 text-xs font-bold rounded-xl transition-all duration-300 ${view === 'calendar' ? 'bg-white shadow-sm text-zinc-900 border border-zinc-150/50' : 'text-zinc-500 hover:text-zinc-700'}`}>Calendar</button>
              </div>
            </div>
          </div>
          <div className="border border-zinc-150/60 rounded-3xl overflow-auto bg-white p-6 shadow-sm relative custom-scrollbar" style={{ height: '460px' }}>
            {view === 'timeline' ? (
        ...
        <section>
           ...
           <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              {subgroups.map(sg => {
                ...
                return (
                  <div key={sg} className="bg-white rounded-[2rem] shadow-[0_10px_40px_rgb(0,0,0,0.03)] border border-zinc-150/50 p-6 lg:p-8 flex flex-col h-full hover:shadow-[0_15px_50px_rgb(0,0,0,0.06)] transition-all duration-300">
                     <div className="flex justify-between items-center mb-6">
                        <h3 className="text-lg font-extrabold text-zinc-900 tracking-tight">{sg}</h3>
                        <span className="text-[10px] font-black uppercase tracking-widest px-3.5 py-1.5 bg-zinc-50 rounded-xl border border-zinc-150/60 text-zinc-600">{avgProgress}% Done</span>
                     </div>
                     ...
                     <div className="space-y-4 flex-1">
                        {sgTasks.map(task => (
                           <div key={task.id} className="p-5 bg-zinc-50/50 rounded-3xl border border-zinc-150/50 hover:bg-white hover:border-violet-300 hover:shadow-[0_8px_30px_rgb(0,0,0,0.05)] transition-all duration-300 group relative">
                              <div className="flex justify-between items-start mb-3">
                                ...
                                {canEditTask(task) && (
                                   <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity bg-white/90 backdrop-blur-md rounded-xl p-1 border border-zinc-150/60 shadow-sm">
                              ...
                              <div className="flex justify-between items-center text-xs font-bold text-zinc-500 mb-5 px-1">
                                 <span className="flex items-center gap-1.5 bg-white px-3 py-1.5 rounded-xl border border-zinc-150/60 shadow-sm"><User size={12} className="text-violet-600"/> {task.assignee}</span>
                                 <span className="text-zinc-400 font-semibold">{formatDate(task.startDate)} - {formatDate(task.endDate)}</span>
                              </div>
                              <div className="flex items-center gap-4 bg-white p-3.5 rounded-2xl border border-zinc-150/60 shadow-sm">
                                 <input 
                              ...
                              <div className="mt-5 pt-4 border-t border-zinc-150/60">
                                  ...
                                  <div className="mt-4 space-y-3">
                                    ...
                                    {userRole.role !== 'guest' && (
                                      <form onSubmit={(e) => { e.preventDefault(); addComment(task.id, e.target.elements.comment.value); e.target.reset(); }} className="flex gap-2 mt-3 relative">
                                        <input name="comment" type="text" placeholder="Write a comment..." className="flex-1 text-sm p-3.5 pr-20 bg-white border border-zinc-150/80 rounded-2xl focus:outline-none focus:ring-2 focus:ring-violet-500/20 focus:border-violet-500 transition-all shadow-sm font-medium" required/>
              ...
              {subgroups.length === 0 && (
                <div className="col-span-full text-center py-20 text-zinc-400 bg-white rounded-[2rem] border-2 border-dashed border-zinc-150">
        ...
        <div className="fixed inset-0 bg-zinc-900/20 backdrop-blur-sm z-[100] flex items-center justify-center p-4 animate-in fade-in duration-200">
          <div className="bg-white rounded-[2rem] shadow-2xl border border-zinc-100 w-full max-w-sm p-8 animate-in zoom-in-95 duration-200">
            ...
            <div className="flex justify-end gap-3">
              {alertConfig.onConfirm && <button onClick={() => setAlertConfig(null)} className="px-5 py-2.5 text-zinc-600 font-bold hover:bg-zinc-50 rounded-2xl transition-colors text-sm border border-transparent hover:border-zinc-150">Cancel</button>}
              <button 
            ...
            <form onSubmit={handleChangePassword} className="space-y-5">
              <div className="relative">
                <input 
                  ...
                  className="w-full p-4 border border-zinc-150 rounded-2xl focus:ring-2 focus:ring-violet-500/20 focus:border-violet-500 outline-none pr-12 transition-all font-semibold text-sm" 
              ...
              <div className="flex justify-end gap-3 mt-8 pt-2">
                <button type="button" onClick={() => setShowPasswordModal(false)} className="px-5 py-2.5 text-zinc-600 font-bold hover:bg-zinc-50 rounded-2xl transition-colors text-sm border border-transparent hover:border-zinc-150">Cancel</button>
      ...
      {showLogin && (
        <div className="fixed inset-0 bg-zinc-900/20 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-in fade-in duration-200">
          <div className="bg-white rounded-[2rem] shadow-2xl border border-zinc-100 w-full max-w-sm p-8 animate-in zoom-in-95 duration-200">
            <div className="w-12 h-12 bg-zinc-50 rounded-2xl flex items-center justify-center mb-6 border border-zinc-150/60 shadow-sm">
            ...
            <form onSubmit={handleLogin} className="space-y-4">
              <div>
                ...
                <input type="text" autoFocus required value={loginUsername} onChange={e => setLoginUsername(e.target.value)} className="w-full p-4 bg-zinc-50 border border-zinc-150 rounded-2xl focus:bg-white focus:ring-2 focus:ring-violet-500/20 focus:border-violet-500 outline-none transition-all font-semibold text-sm" placeholder="e.g. Andres" />
              </div>
              <div className="relative">
                <label className="block text-xs font-bold text-zinc-700 mb-1.5 ml-1 uppercase tracking-wider">Password</label>
                <input type={showLoginPassword ? "text" : "password"} required value={loginPassword} onChange={e => setLoginPassword(e.target.value)} className="w-full p-4 bg-zinc-50 border border-zinc-150 rounded-2xl focus:bg-white focus:ring-2 focus:ring-violet-500/20 focus:border-violet-500 outline-none pr-12 transition-all font-semibold text-sm" placeholder="••••••••"/>
              ...
              <div className="flex justify-end gap-3 mt-8 pt-4 border-t border-zinc-100">
                <button type="button" onClick={() => setShowLogin(false)} className="px-5 py-2.5 text-zinc-600 font-bold hover:bg-zinc-50 rounded-2xl transition-colors text-sm border border-transparent hover:border-zinc-150">Cancel</button>
        ...
        <div className="fixed inset-0 bg-zinc-900/20 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-in fade-in duration-200">
          <div className="bg-white rounded-[2rem] shadow-2xl border border-zinc-100 w-full max-w-md p-8 animate-in zoom-in-95 duration-200">
            ...
            <div className="space-y-3 mb-6 max-h-60 overflow-y-auto pr-2 custom-scrollbar">
              <div className="flex justify-between items-center p-3.5 bg-zinc-50 border border-zinc-150/80 rounded-2xl">
              ...
              {team.map(member => (
                editingUserId === member.id ? (
                  <div key={member.id} className="flex items-center gap-2 p-3 bg-white border-2 border-violet-500 rounded-2xl shadow-sm">
                    <input autoFocus value={editUserName} onChange={e => setEditUserName(e.target.value)} className="flex-1 p-2 text-sm font-bold border border-zinc-150 rounded-xl outline-none focus:border-violet-500" />
                  ...
                  <div key={member.id} className="flex justify-between items-center p-3.5 bg-white border border-zinc-150/80 rounded-2xl hover:border-zinc-300 hover:shadow-sm transition-all group">
                    <div className="flex flex-col">
            ...
            <div className="pt-6 border-t border-zinc-100">
               ...
               <form onSubmit={e => { e.preventDefault(); const t = e.target; addTeamMember(t.name.value, t.role.value); t.reset(); }} className="flex gap-2.5">
                 <input name="name" type="text" placeholder="Name..." className="flex-1 p-3.5 text-sm font-semibold bg-zinc-50 border border-zinc-150 rounded-2xl outline-none focus:bg-white focus:ring-2 focus:ring-violet-500/20 focus:border-violet-500 transition-all" required />
                 <select name="role" className="p-3.5 text-sm font-semibold bg-zinc-50 border border-zinc-150 rounded-2xl outline-none focus:bg-white focus:border-violet-500 transition-all cursor-pointer">
            ...
            <div className="mt-8 flex justify-end">
              <button onClick={() => setShowTeamModal(false)} className="px-6 py-2.5 text-zinc-700 font-bold bg-zinc-50 rounded-2xl border border-transparent hover:border-zinc-150 transition-colors text-sm">Done</button>
            </div>
          </div>
        </div>
      )}
...
function CommentThread({ comments, taskId, userRole, db, appId, tasks, showAlert, parentIds = [] }) {
  ...
  return (
    <div className={`space-y-3 ${parentIds.length > 0 ? 'ml-5 pl-4 border-l-2 border-zinc-150/60' : ''}`}>
      {comments.map((c) => {
        const canEdit = userRole.role === 'manager' || userRole.username === c.author;
        return (
          <div key={c.id}>
            <div className="bg-white p-4 rounded-2xl border border-zinc-150/60 shadow-sm text-sm group/comment relative hover:border-zinc-300 hover:shadow-md transition-all duration-300">
              ...
              {editingCommentId === c.id ? (
                <form onSubmit={e => { e.preventDefault(); saveEdit(c.id, e.target.elements.txt.value); }} className="mt-3 flex gap-2">
                  <input name="txt" defaultValue={c.text} autoFocus className="flex-1 p-2.5 bg-zinc-50 border border-zinc-150 rounded-xl text-sm outline-none focus:border-violet-500 font-semibold"/>
                  <button type="submit" className="px-3 bg-zinc-900 text-white rounded-xl font-bold hover:bg-zinc-800"><Check size={14}/></button>
                  <button type="button" onClick={() => setEditingCommentId(null)} className="px-3 bg-zinc-100 text-zinc-600 rounded-xl font-bold hover:bg-zinc-150"><X size={14}/></button>
                </form>
              ) : (
            ...
            {replyingToId === c.id && (
              <form onSubmit={e => { e.preventDefault(); addReply(c.id, e.target.elements.replyTxt.value); }} className="mt-2.5 flex gap-2 ml-5 pl-4 border-l-2 border-violet-300 relative">
                  ...
                  <div className="absolute right-1.5 top-1.5 bottom-1.5 flex gap-1">
                     ...
                     <button type="button" onClick={() => setReplyingToId(null)} className="px-2.5 bg-white text-zinc-500 border border-zinc-150 rounded-xl hover:bg-zinc-50"><X size={14}/></button>
                  </div>
              </form>
            )}
      ...
      <div className="bg-white rounded-[2rem] shadow-2xl border border-zinc-100 w-full max-w-md p-8 max-h-[90vh] overflow-y-auto animate-in zoom-in-95 duration-200 custom-scrollbar">
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          <div>
            ...
            <input type="text" required value={name} onChange={e => setName(e.target.value)} className="w-full p-4 bg-zinc-50 border border-zinc-150 rounded-2xl focus:bg-white focus:ring-2 focus:ring-violet-500/20 focus:border-violet-500 outline-none transition-all font-semibold text-sm" />
          </div>
          <div>
            <label className="block text-xs font-bold text-zinc-700 mb-1.5 ml-1 uppercase tracking-wider">Subgroup</label>
            <select value={subgroupMode === 'new' ? 'NEW' : subgroupSelect} onChange={e => { if(e.target.value === 'NEW') setSubgroupMode('new'); else { setSubgroupMode('existing'); setSubgroupSelect(e.target.value); } }} className="w-full p-4 bg-zinc-50 border border-zinc-150 rounded-2xl focus:bg-white focus:ring-2 focus:ring-violet-500/20 focus:border-violet-500 outline-none transition-all font-semibold mb-3 cursor-pointer text-sm">
          ...
          <div>
            ...
            <select value={assignee} onChange={e => setAssignee(e.target.value)} className="w-full p-4 bg-zinc-50 border border-zinc-150 rounded-2xl outline-none focus:bg-white transition-all font-semibold cursor-pointer text-sm">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input type="date" required value={startDate} onChange={e => { setStartDate(e.target.value); if (endDate && e.target.value > endDate) setEndDate(e.target.value); }} className="w-full p-4 bg-zinc-50 border border-zinc-150 rounded-2xl outline-none focus:bg-white transition-all font-semibold text-sm" />
            </div>
            <div>
              <label className="block text-xs font-bold text-zinc-700 mb-1.5 ml-1 uppercase tracking-wider">End Date</label>
              <input type="date" required min={startDate} value={endDate} onChange={e => setEndDate(e.target.value)} className="w-full p-4 bg-zinc-50 border border-zinc-150 rounded-2xl outline-none focus:bg-white transition-all font-semibold text-sm" />
            </div>
          </div>
          ...
          <div className="pt-4 border-t border-zinc-100">
             ...
             <div className="flex items-center gap-3 bg-zinc-50 p-3 rounded-2xl border border-zinc-150 w-max">
          ...
          <div className="flex justify-end gap-3 mt-8 pt-4 border-t border-zinc-100">
            <button type="button" onClick={onClose} className="px-5 py-2.5 text-zinc-600 font-bold hover:bg-zinc-50 border border-transparent hover:border-zinc-150 rounded-2xl transition-colors text-sm">Cancel</button>
...
function ChatPanel({ db, appId, userRole, team, showAlert }) {
  ...
  return (
    <div className="fixed bottom-6 right-6 z-[60] flex flex-col items-end">
       {isOpen && (
          <div className="bg-white/95 backdrop-blur-2xl border border-zinc-150/80 rounded-[2rem] shadow-[0_20px_60px_rgba(0,0,0,0.15)] flex flex-col overflow-hidden animate-in slide-in-from-bottom-6 fixed inset-0 sm:relative sm:inset-auto w-full h-full sm:w-[380px] sm:h-[600px] sm:mb-4 z-[70]">
            {!activeChannel ? (
               <div className="flex flex-col h-full">
                 ...
                 <div className="flex-1 overflow-y-auto bg-zinc-50 p-3.5 space-y-2.5 custom-scrollbar">
                    {showCreate && (
                       <div className="bg-white p-4 rounded-2xl border border-zinc-150 mb-4 shadow-sm animate-in fade-in slide-in-from-top-2">
                          ...
                          <div className="space-y-1.5 max-h-40 overflow-y-auto mb-4 custom-scrollbar">
                             {everyoneElse.map(u => (
                                <label key={u} className="flex items-center gap-3 text-sm p-2.5 hover:bg-zinc-50 rounded-xl cursor-pointer transition-colors border border-transparent hover:border-zinc-150 font-semibold text-zinc-700">
                          ...
                          <div className="flex gap-2">
                             <button onClick={() => setShowCreate(false)} className="flex-1 px-3 py-2.5 bg-zinc-100 text-zinc-600 font-bold rounded-xl text-xs hover:bg-zinc-150 transition-colors">Cancel</button>
                    ...
                    {channels.map(ch => {
                       ...
                       return (
                          <button key={ch.id} onClick={() => handleOpenChannel(ch)} className="w-full flex justify-between items-center p-3.5 bg-white rounded-2xl border border-zinc-150/60 shadow-sm hover:shadow-md hover:border-violet-300 transition-all duration-200 text-left group">
               ...
               <div className="flex flex-col h-full bg-zinc-50">
                  <div className="bg-white p-4 flex justify-between items-center shrink-0 pt-[max(env(safe-area-inset-top),16px)] sm:pt-4 border-b border-zinc-100 shadow-sm z-10">
                     <div className="flex items-center gap-3 truncate">
                        <button onClick={() => { setActiveChannel(null); markRead(activeChannel.id); }} className="p-2 bg-zinc-100 hover:bg-zinc-150 text-zinc-700 rounded-xl transition-colors"><ArrowLeft size={16}/></button>
                     ...
                     {messages.map((m, idx) => {
                        ...
                        return (
                           <div key={m.id} className={`flex flex-col ${isMe ? 'items-end' : 'items-start'}`}>
                              ...
                              <div className={`px-4 py-3 rounded-2xl max-w-[85%] text-sm break-words shadow-sm font-medium ${isMe ? 'bg-zinc-900 text-white rounded-br-sm' : 'bg-white border border-zinc-150/80 text-zinc-800 rounded-bl-sm'}`}>
                                 {urlify(m.text)}
                              </div>
                           </div>
                        )
                  ...
                  <form onSubmit={sendMessage} className="p-4 bg-white border-t border-zinc-100 flex gap-2.5 shrink-0 pb-[max(env(safe-area-inset-bottom),16px)] sm:pb-4 rounded-b-[2rem]">
                     <input type="text" value={newMessage} onChange={e => setNewMessage(e.target.value)} placeholder="Type a message..." className="flex-1 px-4 py-3.5 bg-zinc-50 border border-zinc-150 rounded-2xl text-sm outline-none focus:bg-white focus:border-violet-500 focus:ring-2 focus:ring-violet-500/20 transition-all font-medium" />
...
function Timeline({ tasks, zoomLevel, onTaskClick }) {
  ...
  return (
    <div className="relative min-w-max min-h-full pb-8">
      <div className="flex flex-col sticky top-0 bg-white z-20 border-b border-zinc-150/60 shadow-sm">
        
        {/* Months Row - Sticky Horizontally */}
        <div className="flex border-b border-zinc-150/60 bg-zinc-50">
          {monthGroups.map((mg, idx) => (
            <div key={idx} style={{ width: mg.count * dayWidth }} className="flex-shrink-0 border-r border-zinc-150/60 relative">
        ...
        <div className="flex pb-1.5 pt-1.5 bg-white">
          {Array.from({ length: totalDays }).map((_, i) => {
            ...
            return (
              <div key={i} className={`flex-shrink-0 border-r border-zinc-150/40 flex flex-col items-center justify-end pb-1.5 pt-1 ${isToday ? 'bg-violet-100 rounded-t-xl border-violet-300 z-10 relative' : (isWeekend ? 'bg-zinc-150/70' : '')}`} style={{ width: dayWidth }}>
      ...
      <div className="pt-6 space-y-3.5 relative">
        ...
        <div className="absolute inset-0 flex pt-6 pointer-events-none z-0">
          {Array.from({ length: totalDays }).map((_, i) => {
             ...
             const isWeekend = d.getDay() === 0 || d.getDay() === 6;
             return (
                <div key={`bg-${i}`} className={`flex-shrink-0 h-full border-r border-zinc-150/30 ${isWeekend ? 'bg-zinc-150/50' : ''}`} style={{ width: dayWidth }}></div>
             )
          })}
        </div>
  ...
  return (
    <div className="flex flex-col min-h-full pb-8">
      <div className="flex justify-between items-center mb-6 flex-shrink-0 sticky top-0 bg-white z-20 py-2">
        <button onClick={prevMonth} className="p-2.5 hover:bg-zinc-50 rounded-2xl shadow-sm border border-zinc-150/80 transition-all text-zinc-700 font-bold">&lt;</button>
        ...
        <button onClick={nextMonth} className="p-2.5 hover:bg-zinc-50 rounded-2xl shadow-sm border border-zinc-150/80 transition-all text-zinc-700 font-bold">&gt;</button>
      </div>
      
      <div className="bg-zinc-150/60 border border-zinc-150/80 rounded-[1.5rem] overflow-hidden flex flex-col flex-1 shadow-sm">
        <div className="grid grid-cols-7 gap-px bg-zinc-150/60 shrink-0">
        ...
        {weeks.map((week, wIdx) => {
           ...
           return (
              <div key={wIdx} className="grid grid-cols-7 gap-px bg-zinc-150/60 flex-1 min-h-[120px] relative">
                 {week.map((day, dIdx) => (
